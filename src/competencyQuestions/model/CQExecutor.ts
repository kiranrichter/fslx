...
export class CQExecutor {
    ...
    private executeRelation(ontology: Ontology, query: CQValues): CQResult | null {
        const subject = ontology.classes.get(query.subject);
        if (!subject) {
            return null;
        }

        const classes: OntologyClass[] = [subject];
        const relations: GraphRelation[] = [];

        for (const edge of this.edgesOf(subject, query.predicate)) {
        ...
        return this.buildResult(ontology, query.predicate, classes, relations, [subject]);
    }

    private executeShared(ontology: Ontology, query: CQValues): CQResult | null {
        const subjectA = ontology.classes.get(query.subjectA);
        const subjectB = ontology.classes.get(query.subjectB);
        if (!subjectA || !subjectB) {
            return null;
        }

        const classes: OntologyClass[] = [subjectA, subjectB];
        const relations: GraphRelation[] = [];

        for (const edge of this.edgesOf(subjectA, query.predicate)) {
        ...
        return this.buildResult(ontology, query.predicate, classes, relations, [subjectA, subjectB]);
    }
    ...
    private executeClassification(ontology: Ontology, query: CQValues): CQResult | null {
        const subject = ontology.classes.get(query.subject);
        const classification = ontology.classes.get(query.class);
        ...
        return this.buildResult(ontology, undefined, [subject, classification], relations, [subject, classification]);
    }

    private edgesOf(clazz: OntologyClass, predicate?: string): Edge[] {
        const edges: Edge[] = [
        ...
        return predicate ? edges.filter(edge => edge.predicate === predicate) : edges;
    }
    ...
    private buildResult(ontology: Ontology, predicate: string | undefined, classes: OntologyClass[],
                        relations: GraphRelation[], focusClasses: OntologyClass[]): CQResult {
        return {
            graphContext: {
                classes,
                relations,
                visibleRelations: predicate ? [predicate] : ontology.allPredicates
            },
            focusClasses
        };
    }
